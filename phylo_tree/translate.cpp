...
/* change to fit binary output */
    ...
    int int_size2 = int_size << 1;
    int element; // element in a list
    
    int reserve = int_size << 1; // reserved for sequence length and model
    int model = vec_len - 1; 
    // the current model is 11...11 which has the same length of the input sequence length
    
    int ele_start = int_size2*vec_len + reserve; // spaces for the reserved spaces and the header 
    ...
    {
        fout.seekp(int_size2*n+reserve,ios::beg);
        list_size = vt[n].size();
